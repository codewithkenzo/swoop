...
pub struct MemoryStorage {
    ...
    /// Crawl page storage keyed by job_id
    crawl_pages: Arc<DashMap<String, Vec<crate::models::CrawlPage>>>,
    /// Storage statistics
...
impl MemoryStorage {
    ...
    pub fn new() -> Self {
        Self {
            ...
            crawl_pages: Arc::new(DashMap::new()),
...
impl Storage for MemoryStorage {
    ...
    async fn delete_document(&self, id: &str) -> Result<()> {
        ...
        match self.documents.remove(id) {
            ...
            None => {
                ...
                Err(Error::Storage(format!("Document with id '{id}' not found")))
            }
        }
    }
    ...
    async fn store_crawl_page(&self, page: &crate::models::CrawlPage) -> Result<()> {
        ...
        let op_time = start_time.elapsed().as_millis() as f64;
        self.update_stats(true, op_time).await;
        Ok(())
    }
}
...
impl MemoryStorage {
    ...
    /// List crawl pages for a given job id (helper)
    pub async fn list_crawl_pages(&self, job_id: &str) -> Vec<crate::models::CrawlPage> {
        self.crawl_pages
            .get(job_id)
            .map(|entry| entry.value().clone())
            .unwrap_or_default()
