...
async fn performance_comparison() -> Result<()> {
    ...
    println!("   📊 Total sequential time: {sequential_time:?}\n");
            ...
            tokio::spawn(async move {
                for attempt in 1..=3 {
                        ...
                        .await
                    {
                        Ok(response) => {
                            let _response_text = response.text().await.unwrap_or_default();
                            println!("   📊 Batch {batch_id} (attempt {attempt}): Job status received");
                        },
                        Err(e) => println!("   ❌ Batch {batch_id} monitoring error: {e}"),
                    }
...
async fn concurrent_server_ops() -> Result<()> {
    ...
    println!("📊 Completed {successful_ops} concurrent server operations in {total_time:?}");
