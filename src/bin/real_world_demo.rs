...
async fn submit_crawl_job(urls: &[String]) -> Result<String> {
    ...
    if status.is_success() {
        ...
        if let Some(job_id) = json.get("job_id").and_then(|v| v.as_str()) {
            ...
            Err(swoop::error::Error::Parser("No job_id in response".to_string()))
        }
    } else {
        Err(swoop::error::Error::Other(format!("Server error: {status}")))
    }
}
...
async fn monitor_job_progress(job_id: &str) -> Result<()> {
    ...
    for attempt in 1..=5 {
        println!("🔄 Checking job status (attempt {attempt}/5)...");
...
async fn extract_real_data() -> Result<()> {
    ...
    let test_urls = ["https://httpbin.org/html",
